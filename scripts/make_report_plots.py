...
def fig_dataset_overview(run_root, out):
    ...
    ax.set_xticklabels([n.replace("label_", "mask 0x") for n in names],
                       rotation=60, ha="right", fontsize=6.5)
    ax.set_ylabel("runs"); ax.set_title(f"(a) Runs per class ({len(metas)} total)")
    ...
    ax.set_ylabel("runs"); ax.set_title("(b) Imbalance severity (per run)")
    ...
    for pl, colr in zip((0, 5, 10), (C_BLUE, C_ORANGE, C_PURPLE)):
        ...
        ax.scatter([pl] * len(r), r, s=18, color=colr, zorder=3,
                   label=f"{pl} g payload")
        if r:
            ax.plot([pl - 0.6, pl + 0.6], [np.mean(r)] * 2, color=colr, lw=1.5)
    ax.set_xlabel("payload [g]"); ax.set_ylabel("mean hover rotor speed [RPM]")
    ...
    single = [m["gyro_rms"] for m in metas
              if bin(m.get("fault_mask", 0)).count("1") == 1]
    multi = [m["gyro_rms"] for m in metas
             if bin(m.get("fault_mask", 0)).count("1") >= 2]
...
def fig_trajectory(run_root, out):
    ...
    if loaded is None:
        ...
        return
    imu, meta = loaded
    st = np.genfromtxt(os.path.join(run_root, "run_cf2x_healthy_p00_s0",
                                    "state.csv"), delimiter=",", names=True)
    t = st["t"] - st["t"][0]
    ...
    ax.plot(r * np.cos(th), r * np.sin(th), "--", color=C_GRAY, lw=1,
            label="reference circle (r=0.35 m)")
    ax.plot(st["x"], st["y"], color=C_BLUE, lw=1.4, label="flown (sim)")
    ax.scatter([st["x"][0]], [st["y"][0]], marker="o", color=C_GREEN, zorder=5,
               s=28, label="start (on trajectory)")
    ax.set_xlabel("x [m]"); ax.set_ylabel("y [m]")
    ax.set_aspect("equal")
    ax.set_title("(a) Horizontal flight path"); ax.legend(fontsize=7)

    ax = fig.add_subplot(gs[0, 2])
    ax.plot(t, st["z"], color=C_BLUE, lw=1.2)
    ax.axhline(0.55, color=C_GRAY, ls="--", lw=1, label="setpoint 0.55 m")
    ax.fill_between(t, 0.55 - 0.05 * np.sin(w * t), 0.55 + 0.05 * np.sin(w * t),
                    color=C_BLUE, alpha=0.12, label="altitude band")
    ax.set_xlabel("t [s]"); ax.set_ylabel("z [m]")
    ax.set_title("(b) Altitude"); ax.legend(fontsize=7)
    ...
    ax.set_xlabel("t [s]"); ax.set_ylabel("rotor speed [RPM]")
    ...
    ax.set_xlabel("t [s]"); ax.set_ylabel("body rate [rad/s]")
    ax.set_title("(d) Gyroscope"); ax.legend(fontsize=7, ncol=3)

    ax = fig.add_subplot(gs[1, 2])
    ax.plot(t, imu["acc_z"], lw=0.7, color=C_PURPLE)
    ax.axhline(9.81, color=C_GRAY, ls="--", lw=1, label="1 g")
    ax.set_xlabel("t [s]"); ax.set_ylabel(r"$a_z$ [m/s$^2$]")
    ax.set_title("(e) Vertical accelerometer"); ax.legend(fontsize=7)
...
def fig_vibration_signature(run_root, out):
    ...
    specs = [("healthy_p00_s0", "Healthy", C_GREEN),
    ...
    fig, axes = plt.subplots(len(rows), 2, figsize=(10, 3.1 * len(rows)),
                             sharex=True)
    if len(rows) == 1:
        axes = axes[None, :]
    ...
    fig.suptitle("Real 1P vibration signatures: spectral lines appear at each "
                 "faulted rotor's true rotation frequency", y=0.995)
...
def fig_confusion(results, out):
    ...
    ax.set_title("(a) Confusion matrix — run-grouped held-out runs "
                 f"(N={len(trues)} windows)", fontsize=10)
    thresh = 0.5
    for i in range(n):
        for j in range(n):
            v = cmn[i, j]
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        f1 = 2 * cm.diagonal() / np.maximum(2 * cm.diagonal() +
                                            cm.sum(axis=0) - cm.diagonal() +
                                            cm.sum(axis=1) - cm.diagonal(), 1)
    order = np.argsort(f1)
    cols = [C_GREEN if names[i] == "healthy" else (C_RED if f1[i] < 0.9 else C_BLUE)
            for i in order]
    ax.barh([names[i] for i in order], f1[order], color=cols)
    ax.axvline(f1.mean(), ls="--", color=C_GRAY, lw=1,
               label=f"macro F1 = {f1.mean():.3f}")
...
def fig_benchmark(results, out):
    ...
    ax1.axhline(float(summ.get("overall_accuracy", 0)) * 100, ls="--",
                color=C_RED, lw=1.2, label=f"overall = {float(summ.get('overall_accuracy',0))*100:.1f}%")
    ax1.set_xticks(range(len(rows)))
    ax1.set_xticklabels([n.replace("label_", "m") for n in names],
                        rotation=60, ha="right", fontsize=6.5)
    ax1.set_ylim(0, 112); ax1.set_ylabel("accuracy [%]")
    ...
    ax2.axhline(float(summ.get("p99_latency_ms", 0)), ls="--", color=C_RED,
                lw=1.2, label=f"p99 = {float(summ.get('p99_latency_ms',0)):.2f} ms")
    ax2.set_xticks(range(len(rows)))
    ax2.set_xticklabels([n.replace("label_", "m") for n in names],
                        rotation=60, ha="right", fontsize=6.5)
