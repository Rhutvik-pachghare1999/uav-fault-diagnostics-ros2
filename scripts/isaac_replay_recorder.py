...
os.environ.setdefault("EXP_PATH", _REPO_ROOT)
os.environ.setdefault("CARB_APP_PATH", os.path.expanduser("~/isaacsim"))
...
os.environ.setdefault(
    "QUAD_USD_PATH",
    os.environ.get("QUAD_USD_PATH", os.path.join(_REPO_ROOT, "isaac", "assets", "Bitcraze", "Crazyflie", "cf2x.usd")),
)
os.environ.setdefault(
    "ISAAC_OUTDIR", os.environ.get("ISAAC_OUTDIR", os.path.join(os.environ["PROJECT_ROOT"], "isaac_dataset"))
)
...
def parse_args():
    ...
    p.add_argument(
        "--duration", type=float, default=20.0, help="seconds (will be increased if needed to reach min-rows)"
    )
    ...
    p.add_argument(
        "--use-fallback", action="store_true", help="force fallback synthetic data generator (skip Isaac imports)"
    )
    return p.parse_args()
...
def generate_synthetic(outdir, run_name, duration, fps, min_rows, apply_fault=False, fault_motor=1, ur=0.0):
    ...
    with (
        ...
        th_writer.writerow(
            ["time", "thrust1", "thrust2", "thrust3", "thrust4", "torque1", "torque2", "torque3", "torque4"]
        )
        ...
        for step in range(steps):
            ...
            target_rpms = [
                base + (i * 20.0) + (5.0 * math.sin(2 * math.pi * 0.5 * (i + 1) * t)) + random.uniform(-10, 10)
                for i in range(4)
            ]
            if apply_fault:
...
def run_isaac_recording(args):
    ...
    if args.rotor_prims:
        ...
        cand = (
            args.robot_prim
            if stage.GetPrimAtPath(args.robot_prim) and stage.GetPrimAtPath(args.robot_prim).IsValid()
            else "/World"
        )
        for n in names:
    ...
    if adjusted_duration > args.duration:
        print(
            f"[INFO] Increased duration from {args.duration:.3f}s -> {adjusted_duration:.3f}s "
            f"to meet min_rows={args.min_rows}"
        )
        ...
        for step in range(steps):
            ...
            try:
                ...
                if imu_prim:
                    ...
                    if a and a.IsValid():
                        val = a.Get()
                        acc = tuple(val) if val else acc
                    a2 = imu_prim.GetAttribute("angular_velocity")
                    if a2 and a2.IsValid():
                        val2 = a2.Get()
                        gyro = tuple(val2) if val2 else gyro
            except Exception:
                pass
    ...
    meta = {
        ...
        "rotor_candidates": rotor_prims,
        "rotor_exists": exist_map,
    }
...
def main():
    ...
    if desired_steps > int(math.ceil(args.duration * args.fps)):
        ...
        print(
            f"[MAIN] Extending duration {args.duration:.3f}s -> {new_duration:.3f}s to reach min_rows={args.min_rows}"
        )
        args.duration = new_duration

    if args.use_fallback:
        run = generate_synthetic(
            args.outdir, run_name, args.duration, args.fps, args.min_rows, args.apply_fault, args.fault_motor, args.ur
        )
        print("Done (fallback).")
        return
