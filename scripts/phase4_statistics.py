...
def get_condition_bins():
    return {
        ...
        "cg_bias": {
            "none": [(0.0, 0.0)],
            "low": [(0.01, 0.01), (-0.01, -0.01), (0.01, -0.01), (-0.01, 0.01)],
            "high": [(0.02, 0.02), (-0.02, -0.02), (0.02, -0.02), (-0.02, 0.02)],
        },
...
def run_baseline_vs_mmd_experiment(seed=42):
    ...
    for rid in np.unique(run_id):
        ...
        if payload in [1.0, 1.2]:
            payload_bin = "light"
        elif payload in [1.5, 1.8]:
            payload_bin = "medium"
        else:
            payload_bin = "heavy"
    ...
    for split_name, train_cond, test_cond in splits:
        ...
        if baseline and mmd_result:
            ...
            print(
                f"  {split_name}: Baseline Acc={baseline['test_acc']:.4f}, "
                f"MMD Acc={mmd_result['test_acc']:.4f}, ΔAcc={delta_acc:+.4f}"
            )
...
def run_multi_seed_experiment(n_seeds=5, seeds=None):
    ...
    for split_name in splits:
        ...
        baseline_accs = [
            all_results[s][split_name]["baseline"]["test_acc"] for s in seeds if split_name in all_results[s]
        ]
