...
def read_run(run_dir, window=100, step=1, vars=DEFAULT_VARS,
             allow_missing_vars=False, allow_missing_runs=False):
    ...
    if missing:
        if allow_missing_vars:
            print(f"WARNING: {run_dir}: zero-filling missing vars {missing} "
                  f"(model will see dead channels)")
            for v in missing:
                df[v] = 0.0
        else:
            raise ValueError(
...
def build_dataset(project_root: str, out_h5: str, window:int=100, step:int=1, vars=DEFAULT_VARS,
    ...
    if not all_X:
        ...
        raise SystemExit(
            "No windows found in any run. Check ISAAC dataset directory and "
            "imu.csv placement.")
...
if __name__ == "__main__":
    ...
    p.add_argument("--allow-missing-vars", action="store_true",
