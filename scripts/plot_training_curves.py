...
def main():
    ...
    if len(e):
        plt.figure()
        plt.plot(e, tl, label="train_loss")
        plt.xlabel("epoch")
        plt.ylabel("loss")
        plt.grid(True)
        plt.legend()
        plt.savefig(os.path.join(args.out, "train_loss.png"))
    if len(va):
        plt.figure()
        plt.plot(np.arange(1, len(va) + 1), va, label="val_acc")
        plt.xlabel("epoch")
        plt.ylabel("val_acc")
        plt.grid(True)
        plt.legend()
        plt.savefig(os.path.join(args.out, "val_acc.png"))
    if len(vl):
        plt.figure()
        plt.plot(np.arange(1, len(vl) + 1), vl, label="val_loss")
        plt.xlabel("epoch")
        plt.ylabel("val_loss")
        plt.grid(True)
        plt.legend()
        plt.savefig(os.path.join(args.out, "val_loss.png"))
