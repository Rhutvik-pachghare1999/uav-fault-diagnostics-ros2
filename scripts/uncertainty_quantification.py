...
def evaluate_uncertainty(args):
    ...
    if args.split == "train" and not args.allow_train_eval:
        raise SystemExit(
            "refusing to evaluate on the TRAIN split (training "
    ...
    if model_vars and h5_vars and list(model_vars) != list(h5_vars):
        ...
        X = X[:, :, col_idx, :]
        print(f"  channel alignment: using {len(col_idx)} of {len(h5_vars)} channels ({','.join(model_vars)})")
    ...
    model = PaperCNN(in_channels=1, base_filters=base_filters, num_classes=n_faults).to(device)
    ...
    for class_id in sorted(np.unique(true)):
        ...
        if mask.sum() > 0:
            ...
            print(
                f"  {class_name}: Acc={accuracy_score(true[mask], preds[mask]):.4f}, "
...
def main():
    ...
    parser.add_argument("--allow-train-eval", action="store_true", help="explicitly allow evaluating the train split")
    parser.add_argument(
        "--allow-unsealed", action="store_true", help="allow legacy datasets without split/is_aug columns"
    )
