...
class PaperCNN(nn.Module):
    ...
    def __init__(self, in_channels=1, base_filters=32, num_classes=16):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels, f, kernel_size=3, padding=1), nn.BatchNorm2d(f), nn.ReLU(), nn.MaxPool2d((2, 2))
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(f, f * 2, 3, padding=1), nn.BatchNorm2d(f * 2), nn.ReLU(), nn.MaxPool2d((2, 2))
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(f * 2, f * 4, 3, padding=1), nn.BatchNorm2d(f * 4), nn.ReLU(), nn.AdaptiveAvgPool2d((1, 1))
        )
