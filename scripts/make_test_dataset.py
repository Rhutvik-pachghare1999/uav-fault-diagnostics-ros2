#!/usr/bin/env python3
"""Generate synthetic dataset with run_id for testing leakage fix."""
import os, json, numpy as np, h5py, pandas as pd
...
def make_synthetic_imu(run_id, fault_mask, n_samples=500, window=100, step=10):
    ...
    # Base RPMs around 3000 with noise
    rpm_base = 3000 + 50 * np.sin(0.5 * t)
    rpm = np.tile(rpm_base, (4, 1)).T
    
    # Apply fault: reduce RPM on faulty props
    ...
    # Accel and gyro with small vibrations + noise
    acc = np.random.normal(0, 0.5, (T, 3))
    gyro = np.random.normal(0, 0.05, (T, 3))
    
    # Add vibration at RPM frequency to accel
    ...
    df = pd.DataFrame({
        ...
        'rpm1': rpm[:, 0], 'rpm2': rpm[:, 1], 'rpm3': rpm[:, 2], 'rpm4': rpm[:, 3],
        'roll': roll, 'pitch': pitch, 'yaw': yaw,
    })
    
    return df

def build_synthetic_dataset(n_runs=8, n_windows_per_run=40, window=100, step=10, out_h5="ml_dataset_test.h5"):
    """Build synthetic dataset with run_id."""
    ...
    # Create 8 runs with different fault masks (0-7, so 8 classes)
    for run_id in range(n_runs):
        fault_mask = run_id % 8  # 8 different fault classes
    ...
    X = np.stack([r[0] for r in runs], axis=0).astype("float32")  # (N, C, W)
    X = X[:, None, :, :]  # (N, 1, C, W)
...
if __name__ == "__main__":
    build_synthetic_dataset(n_runs=8, out_h5="ml_dataset_test.h5")