...
def _box(ax, x, y, w, h, title, lines, fill, edge, fs_title=9.5, fs_body=8, title_color=NAVY, lw=1.6, r=0.02):
    ax.add_patch(
        FancyBboxPatch((x, y), w, h, boxstyle=f"round,pad=0.6,rounding_size={r}", fc=fill, ec=edge, lw=lw, zorder=2)
    )
    ax.text(
        x + w / 2,
        y + h - 1.6,
    ...
    ty = y + h - 4.4
    for ln in lines:
        ax.text(x + 1.3, ty, ln, ha="left", va="top", fontsize=fs_body, color=INK, zorder=3)
        ty -= 2.5
    return ty
...
def _arrow(ax, x0, x1, y, color=GRAY, lw=2.2, style="-|>", ms=22):
    ax.add_patch(
        FancyArrowPatch(
            (x0, y), (x1, y), arrowstyle=style, mutation_scale=ms, color=color, lw=lw, zorder=1, shrinkA=0, shrinkB=0
        )
    )
...
def _chip(ax, x, y, w, text, edge="#8896A6", fs=7.2):
    ax.add_patch(
        FancyBboxPatch((x, y), w, 3.2, boxstyle="round,pad=0.35,rounding_size=0.5", fc=WHITE, ec=edge, lw=1.1, zorder=2)
    )
    ax.text(x + w / 2, y + 1.6, text, ha="center", va="center", fontsize=fs, family="monospace", color=NAVY, zorder=3)


def _title(ax, x, y, text, sub=None, fs=13, fs_sub=9.5):
    ax.text(x, y, text, ha="center", va="top", fontsize=fs, fontweight="bold", color=NAVY)
    if sub:
        ax.text(x, y - 2.6, sub, ha="center", va="top", fontsize=fs_sub, color=GRAY)
...
def fig_system_architecture(out):
    ...
    _title(
        ...
        60,
        52,
        "UAV Aegis — End-to-End Fault Diagnostics Pipeline",
    ...
    stages = [
        (
            "1 · SIMULATION",
            [
                "Isaac Sim 5.1.0 + PhysX",
                "Crazyflie 2.1 X-quad (USD)",
                "IMU @ 500 Hz · 9 s missions",
        ...
        (
            "3 · TRAINING",
            [
                "PaperCNN · 112K params",
                "train / val / test = disjoint",
                "flights (GroupShuffleSplit)",
                "50 epochs · early stop @ 22",
                "best val acc 0.9931",
            ],
        ),
        (
            "4 · EVALUATION",
            [
                "test acc 99.0% · macro F1 0.99",
                "healthy F1 1.00",
                "cross-RPM bins 98.8–99.5%",
                "MC-dropout entropy 0.0010",
        ...
        (
            ...
            [
                "ROS2 Jazzy node",
                "live /imu/data @ 500 Hz",
                "or CSV flight-log replay",
                "publishes /fault_detection",
    ...
    chips = [
        "isaac_dataset/",
        "ml_sealed.h5",
        "models/cnn_sealed.pth",
        "results/ · evals + figures",
        "/fault_detection topic",
    ]
...
def fig_model_architecture(out):
    ...
    _title(
        ...
        60,
        40,
        f"PaperCNN — Fault Classifier ({n_params:,} parameters)",
    ...
    blocks = [
        (
            "Input",
            "1 × 13 × 100",
            ["rpm1-4 · roll · pitch · yaw", "gyro xyz · acc xyz", "normalized (checkpoint", "mean / std)"],
            STAGES[1],
        ),
        (
            "Conv Block 1",
            "32 × 6 × 50",
            ["Conv2d 3×3 → 32", "BatchNorm · ReLU", "MaxPool 2 × 2", "— 384 params"],
            STAGES[2],
        ),
        (
            "Conv Block 2",
            "64 × 3 × 25",
            ["Conv2d 3×3 → 64", "BatchNorm · ReLU", "MaxPool 2 × 2", "— 18,624 params"],
            STAGES[2],
        ),
        (
            "Conv Block 3",
            "128 × 1 × 1",
            ["Conv2d 3×3 → 128", "BatchNorm · ReLU", "Global Avg Pool", "— 74,112 params"],
            STAGES[2],
        ),
        ("FC", "128", ["Flatten 128", "Linear → 128", "ReLU", "— 16,512 params"], STAGES[3]),
        ("Head", "16 logits", ["Linear → 16", "healthy + 15 rotor", "fault combinations", "— 2,064 params"], STAGES[3]),
    ]
    for (title, shape, lines, (fill, edge)), x in zip(blocks, xs):
        ax.add_patch(
            FancyBboxPatch(
                (x, Y), W, H, boxstyle="round,pad=0.6,rounding_size=0.02", fc=fill, ec=edge, lw=1.6, zorder=2
            )
        )
        ax.text(
            x + W / 2, Y + H - 1.3, title, ha="center", va="top", fontsize=9.5, fontweight="bold", color=NAVY, zorder=3
        )
        ax.text(
            x + W / 2, Y + H - 5.0, shape, ha="center", va="top", fontsize=9, fontweight="bold", color=edge, zorder=3
        )
        ty = Y + H - 7.8
        for ln in lines:
            ax.text(x + W / 2, ty, ln, ha="center", va="top", fontsize=7.3, color=INK, zorder=3)
            ty -= 2.3
...
def fig_fault_injection(out):
    ...
    fig.suptitle(
        "Physical Fault Injection — Crazyflie 2.1 X-Quad (Isaac Sim + PhysX)",
        fontsize=13,
        fontweight="bold",
        color=NAVY,
        x=0.5,
    ...
    axA.axis("off")
    axA.set_xlim(-5.4, 5.4)
    axA.set_ylim(-4.9, 4.9)
    axA.set_title("Rotor bitmask → 15 fault combinations + healthy", fontsize=9.5, color=NAVY, pad=2)
    ...
    axA.text(cx, cy, "CF 2.1", ha="center", va="center", fontsize=8, color="white", fontweight="bold", zorder=4)
    ...
    for rx, ry, name, hx, d, pos in rotors:
        ...
        axA.add_patch(
            Circle(
                (rx, ry), 0.72, fc="#FFFFFF" if not faulted else "#FDECEC", ec=col, lw=2.4 if faulted else 1.8, zorder=2
            )
        )
        axA.text(rx, ry + 0.14, name, ha="center", va="center", fontsize=9.5, fontweight="bold", color=col, zorder=4)
        axA.text(rx, ry - 0.33, hx, ha="center", va="center", fontsize=7, family="monospace", color=GRAY, zorder=4)
        # rotation-direction arc
        th = np.linspace(0.35, 2.6, 24) * d
        arc_r = 1.05
        axA.plot(rx + arc_r * np.cos(th), ry + arc_r * np.sin(th), color=col, lw=1.2, alpha=0.85, zorder=1)
        arr_th = 2.6 * d
        axA.annotate(
            "",
            xy=(rx + arc_r * np.cos(arr_th + 0.14 * d), ry + arc_r * np.sin(arr_th + 0.14 * d)),
    ...
    axA.annotate("↑ front", xy=(0, 3.6), ha="center", fontsize=8.5, color=NAVY, fontweight="bold")
    axA.annotate(
        "example: mask 0x03 = R1 + R2 faulted → class label_3 (severity: Severe)",
        xy=(0, -3.9),
        ha="center",
        fontsize=8,
        color=GRAY,
    ...
    axB.set_ylim(0, 10)
    _box(
        axB, 0.2, 0.2, 9.6, 9.4, "Fault profile — applied to each faulted rotor", [], "#FDEEE1", "#E8722A", fs_title=10
    )
    profile = [
    ...
    axC.set_xlim(0, 10)
    axC.set_ylim(0, 10)
    _box(axC, 0.2, 0.2, 9.6, 9.4, "Signatures measured in the flight data", [], "#E8F6EC", "#2CA02C", fs_title=10)
    effects = [
...
def fig_ros2_deployment(out):
    ...
    _title(
        ...
        60,
        48,
        "ROS2 Jazzy Deployment — uav_aegis · fault_inference_node",
    ...
    _box(
        ...
        26,
        11,
        "LIVE  ·  sensor_msgs/Imu",
        ["/imu/data @ 500 Hz", "acc xyz · gyro xyz", "rpy from orientation", "rpm channels zero-filled"],
        STAGES[0][0],
        STAGES[0][1],
        fs_title=9,
    )
    _box(
        ...
        26,
        11,
        "REPLAY  ·  CSV flight log",
        ["isaac_dataset/run_*/imu.csv", "13 columns @ 500 Hz", "offline smoke-testing", "--mode replay --step N"],
        STAGES[1][0],
        STAGES[1][1],
        fs_title=9,
    )
    ...
    ax.add_patch(
        FancyBboxPatch(
            (nx, 6), nw, 34, boxstyle="round,pad=0.8,rounding_size=0.03", fc="#F0ECF8", ec="#7B5EA7", lw=2.2, zorder=2
        )
    )
    ax.text(
        nx + nw / 2,
        37.6,
        "fault_inference_node",
        ha="center",
        va="top",
        fontsize=11,
        fontweight="bold",
    ...
    for k, v in steps:
        ...
        ax.text(nx + 5, y - 3.1, k, ha="left", va="center", fontsize=8.6, fontweight="bold", color=NAVY, zorder=4)
        ax.text(nx + 21, y - 3.1, v, ha="left", va="center", fontsize=7.8, color=GRAY, zorder=4)
        y -= 6.4
    ...
    ax.text(105, 21.5, "severity with --publish-severity", ha="center", fontsize=7.2, color=GRAY, style="italic")
    ...
    ax.text(
        60,
        2.8,
        "replay mode verified on held-out flights: healthy → healthy · mask01 → label_1 · "
        "mask0f → label_15 (9/9 windows each)",
        ha="center",
        va="center",
        fontsize=8.5,
        color=GRAY,
...
def verify(path):
    ...
    print(
        f"  {path.name:<32} {path.stat().st_size / 1024:7.1f} KB  "
