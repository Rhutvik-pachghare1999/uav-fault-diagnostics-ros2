...
Safety rules (added after the deployment audit):
  ...
  * Live mode validates the IMU stream: if the measured sample rate deviates
    from --expected-rate (default 500 Hz, the dataset rate) by more than
    --rate-tolerance, predictions are annotated with the measured rate and an
    error is logged — a 100 Hz stream fed into 100-sample windows would span
    1 s instead of the 0.2 s the model was trained on.
  * If the model consumes roll/pitch/yaw, IMU messages with invalid
    orientation (covariance[0] < 0 per REP 1203, or a non-unit quaternion)
    are rejected — inference suspends rather than feeding garbage euler
    angles.
...
IMU_CHANNELS = ("acc_x", "acc_y", "acc_z", "gyro_x", "gyro_y", "gyro_z", "roll", "pitch", "yaw")
...
def main():
    ...
    p.add_argument(
        ...
        type=float,
        default=1.0,
        help="max age (s) of cached RPM values before inference is suspended in live mode",
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        help="replay escape hatch: zero-fill missing CSV columns (predictions are marked compromised)",
    )
    p.add_argument(
        "--expected-rate",
        type=float,
        default=500.0,
        help="expected IMU sample rate in Hz (the dataset is 500 Hz); "
        "live windows are validated against this. 0 disables "
        "the check.",
    )
    p.add_argument(
        "--rate-tolerance",
        type=float,
        default=0.25,
        help="allowed relative deviation of the measured IMU rate "
        "from --expected-rate before predictions are marked "
        "compromised",
    )
    ...
    if not var_list:
        raise SystemExit(
            "checkpoint meta has no 'vars' channel list — cannot "
    ...
    if unknown:
        raise SystemExit(
            f"model consumes channels this node cannot produce: {unknown}. "
            f"Supported: {list(RPM_CHANNELS + IMU_CHANNELS)}"
        )
    needs_rpm = any(v.startswith("rpm") for v in var_list)
    needs_euler = any(v in ("roll", "pitch", "yaw") for v in var_list)
    ...
    def run_inference(X_win):
        ...
        if X.shape[0] == args.window and X.shape[1] == n_ch:
            ...
            raise ValueError(f"Unexpected input shape: {X.shape} (expected ({n_ch}, {args.window}))")
    ...
    class InferenceNode(Node):
        def __init__(self):
            ...
            self.stamps = collections.deque(maxlen=51)
            self.measured_rate = None
            self.rate_mismatch = False
            self.rate_warn_count = 0
            self.orient_rejects = 0
            self.nan_rejects = 0
            self.sub = self.create_subscription(Imu, args.topic, self.cb_imu, 10)
            if needs_rpm:
                self.rpm_sub = self.create_subscription(Float32MultiArray, args.rpm_topic, self.cb_rpm, 10)
            ...
            if args.expected_rate > 0:
                self.get_logger().info(
                    f"Expecting ~{args.expected_rate:.0f} Hz on {args.topic} "
                    f"(tolerance {args.rate_tolerance * 100:.0f}%)"
                )
            if needs_euler:
                self.get_logger().info(
                    "Model consumes roll/pitch/yaw: IMU orientation must be valid (covariance[0] >= 0, unit quaternion)"
                )
            self.sample_count = 0

        def cb_rpm(self, msg: Float32MultiArray):
            if len(msg.data) != 4:
                self.get_logger().warn(f"expected 4 rotor RPMs on {args.rpm_topic}, got {len(msg.data)}")
                return
            self.rpm_cache = [float(v) for v in msg.data]
            self.rpm_time = time.monotonic()

        def cb_imu(self, msg: Imu):
            # NaN sensor values poison whole windows: drop the sample.
            vals = (
            ...
            if any(v != v for v in vals):
                self.nan_rejects += 1
                if self.nan_rejects == 1 or self.nan_rejects % 500 == 0:
                    self.get_logger().error(
                        f"NaN in IMU sample on {args.topic} — sample dropped ({self.nan_rejects} rejected so far)"
                    )
                return

            # Orientation validity: only enforceable requirement when the
            # model consumes roll/pitch/yaw. REP 1203 marks unknown
            # orientation with covariance[0] < 0; zero/non-unit quaternions
            # are equally useless. Rejecting suspends inference instead of
            # feeding the model fabricated euler angles.
            q = msg.orientation
            if needs_euler:
                qnorm = math.sqrt(q.x * q.x + q.y * q.y + q.z * q.z + q.w * q.w)
                invalid = msg.orientation_covariance[0] < 0.0 or qnorm != qnorm or abs(qnorm - 1.0) > 0.1
                if invalid:
                    self.orient_rejects += 1
                    if self.orient_rejects == 1 or self.orient_rejects % 500 == 0:
                        self.get_logger().error(
                            f"IMU orientation invalid on {args.topic} "
                            f"(covariance[0] < 0 or non-unit quaternion) — "
                            f"INFERENCE SUSPENDED: this model consumes "
                            f"roll/pitch/yaw ({self.orient_rejects} samples "
                            f"rejected so far)"
                        )
                    return

            # Sample-rate validation: the model was trained on 100-sample
            # windows at the dataset rate; a slower stream stretches the
            # window duration and shifts every frequency-domain signature.
            stamp = msg.header.stamp
            if stamp.sec > 0 or stamp.nanosec > 0:
                t_msg = stamp.sec + stamp.nanosec * 1e-9
            else:
                t_msg = time.monotonic()
            if self.stamps and not (0.0 < t_msg - self.stamps[-1] <= 10.0):
                self.stamps.clear()  # clock discontinuity: re-seed the median
            self.stamps.append(t_msg)

            self.rate_mismatch = False
            if args.expected_rate > 0 and len(self.stamps) == self.stamps.maxlen:
                dts = [b - a for a, b in zip(list(self.stamps), list(self.stamps)[1:])]
                med_dt = sorted(dts)[len(dts) // 2]
                if med_dt > 0:
                    self.measured_rate = 1.0 / med_dt
                    tol = args.rate_tolerance * args.expected_rate
                    if abs(self.measured_rate - args.expected_rate) > tol:
                        self.rate_mismatch = True
                        self.rate_warn_count += 1
                        if self.rate_warn_count == 1 or self.rate_warn_count % 100 == 0:
                            self.get_logger().error(
                                f"IMU rate {self.measured_rate:.1f} Hz deviates "
                                f"from expected {args.expected_rate:.1f} Hz — a "
                                f"{self.window_size}-sample window now spans "
                                f"{self.window_size / self.measured_rate:.2f}s "
                                f"instead of "
                                f"{self.window_size / args.expected_rate:.2f}s; "
                                f"predictions marked rate_mismatch"
                            )
            ...
            if needs_rpm:
                stale = self.rpm_cache is None or time.monotonic() - self.rpm_time > args.rpm_max_age
                if stale:
                    if not self.rpm_stale_warned or self.sample_count % 500 == 0:
                        self.get_logger().error(
                            f"no fresh RPM telemetry on {args.rpm_topic} — INFERENCE SUSPENDED (would be out-of-domain)"
                        )
            ...
            if len(self.win) == self.window_size:
                ...
                if self.rate_mismatch:
                    pred["rate_mismatch_hz"] = round(self.measured_rate, 1)
                out_str = str(pred)
