...
def main():
    ...
    np.savez(os.path.join(args.out, 'predictions.npz'),
             trues=np.array(trues, dtype='int64'),
             preds=np.array(preds, dtype='int64'),
             sev=np.array(trues_sev, dtype='int64') if trues_sev else np.array([], dtype='int64'),
             label_names=np.array(labels_sorted),
             class_idx=np.array(sorted(list(set(trues) | set(preds))), dtype='int64'))
