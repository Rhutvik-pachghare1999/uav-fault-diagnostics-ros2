...
def plot_sample_signal(X, true_label, pred_label, label_map, outpath):
    ...
    ax[-1].set_xlabel("t")
    fig.suptitle(
        f"True: {label_map.get(true_label, str(true_label))} Pred: {label_map.get(pred_label, str(pred_label))}"
    )
    fig.tight_layout()
    fig.savefig(outpath, dpi=150)
    plt.close(fig)
...
def main():
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="optional: subsample WITHIN the selected split only (0 = use every window of the split)",
    )
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--batch-size", type=int, default=1024)
    p.add_argument(
        "--allow-train-eval", action="store_true", help="escape hatch for diagnostics; NEVER for reported metrics"
    )
    args = p.parse_args()
    if args.split == "train" and not args.allow_train_eval:
        raise SystemExit(
            "Refusing to evaluate on the TRAIN split without "
    ...
    with h5py.File(args.h5, "r") as f:
        ...
        if model_sha and h5_sha and model_sha != h5_sha:
            raise SystemExit(
                f"MANIFEST MISMATCH: model trained on manifest "
        ...
        y_sev = f["y_sev"][sorted(idxs)][np.argsort(idxs, kind="stable")] if "y_sev" in f else None
        run_id = f["run_id"][sorted(idxs)][np.argsort(idxs, kind="stable")] if "run_id" in f else None
    ...
    if run_id is not None:
        ...
        run_mean = float(np.mean(accs))
        run_std = float(np.std(accs))
        run_min = float(np.min(accs))
    else:
    ...
    with open(os.path.join(args.out, "classification_report.txt"), "w") as f:
        f.write(report)
    np.savez(
        os.path.join(args.out, "predictions.npz"),
        trues=y,
        preds=preds,
        sev=y_sev if y_sev is not None else np.array([], dtype="int64"),
        run_id=run_id if run_id is not None else np.array([], dtype="int64"),
        label_names=np.array(label_names),
        class_idx=np.array(labels_used, dtype="int64"),
    )
    eval_meta = {
    ...
    print(f"[{args.split}] windows={len(X)}  window_acc={win_acc:.4f}  balanced_acc={bal_acc:.4f}")
    if run_mean is not None:
        print(f"per-run accuracy: mean={run_mean:.4f} std={run_std:.4f} min={run_min:.4f} over {len(per_run)} runs")
