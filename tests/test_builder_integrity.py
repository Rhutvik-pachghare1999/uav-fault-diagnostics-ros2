...
FULL_VARS = ["rpm1", "rpm2", "rpm3", "rpm4", "roll", "pitch", "yaw",
...
def make_run(root, name, columns, n_rows=120, with_imu=True):
    ...
    if with_imu:
        df = pd.DataFrame(
            {c: np.random.randn(n_rows) for c in columns})
...
def test_read_run_missing_vars_escape_hatch_zero_fills(tmp_path, capsys):
    ...
    Xs, metas = builder.read_run(str(tmp_path / "run_b"), vars=FULL_VARS,
...
def test_read_run_missing_imu_escape_hatch_skips(tmp_path):
    ...
    Xs, metas = builder.read_run(str(tmp_path / "run_d"), vars=FULL_VARS,
