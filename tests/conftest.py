...
VARS13 = ["rpm1", "rpm2", "rpm3", "rpm4", "roll", "pitch", "yaw",
          "gyro_x", "gyro_y", "gyro_z", "acc_x", "acc_y", "acc_z"]
VARS9 = ["roll", "pitch", "yaw", "gyro_x", "gyro_y", "gyro_z",
         "acc_x", "acc_y", "acc_z"]
FAULT_CLASSES = ["healthy"] + [f"label_{i}" for i in range(1, 16)]
PAYLOADS = [0.0, 0.005, 0.01]


def make_checkpoint(path, vars_list, n_faults=16, manifest_sha=None,
                    base_filters=32, extra_meta=None):
...
def manifest(synth_root, tmp_path_factory):
    ...
    run_script("make_split_manifest.py",
               "--project-root", str(synth_root), "--out", str(out))
    return out
...
def source_h5(synth_root, tmp_path_factory):
    ...
    run_dirs = sorted(glob.glob(os.path.join(str(synth_root),
                                             "isaac_dataset", "run_*")))
    ...
    with h5py.File(out, "w") as f:
        ...
        f.attrs["meta"] = json.dumps({
            "vars": VARS13, "window": 100, "step": 25,
            "fault_label_map": {c: i for i, c in enumerate(FAULT_CLASSES)},
        })
    return out
...
def sealed_h5(synth_root, source_h5, manifest, tmp_path_factory):
    ...
    run_script("build_sealed_dataset.py",
               "--h5", str(source_h5), "--manifest", str(manifest),
               "--out", str(out), "--project-root", str(synth_root),
...
def sealed_ckpt(sealed_h5, manifest, tmp_path_factory):
    ...
    run_script("train_cnn.py",
               "--h5", str(sealed_h5), "--manifest", str(manifest),
               "--out", str(out), "--epochs", "1",
               "--history", str(out.with_suffix(".history.json")))
    return out
