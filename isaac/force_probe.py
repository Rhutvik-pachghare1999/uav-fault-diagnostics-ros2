...
try:
    ...
    cube = world.scene.add(
        DynamicCuboid(prim_path="/World/cube", scale=[0.1, 0.1, 0.1], mass=1.0, position=np.array([0, 0, 1.0]))
    )
    ...
    world.reset()
    art.initialize()
    art.set_world_pose(position=np.array([0.0, 0.0, 1.0]), orientation=np.array([1.0, 0.0, 0.0, 0.0]))
    ...
    for i in range(250):
        physx.apply_force_at_pos(stage_id, cube_pid, carb.Float3(0, 0, 5.0), carb.Float3(0, 0, 0), "Force")
        physx.apply_force_at_pos(stage_id, body_pid, carb.Float3(0, 0, 0.5), carb.Float3(0, 0, 0), "Force")
        world.step(False)
